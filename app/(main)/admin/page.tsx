            ...
            <div className="max-w-6xl mx-auto space-y-8">
                <div className="flex items-center gap-4 border-b border-border pb-6">
                    <button
                        ...
                        className="p-3 rounded-xl bg-muted text-muted-foreground hover:text-foreground hover:bg-muted/80 transition"
                    >
                    ...
                    <div>
                        ...
                        <p className="text-muted-foreground">System Controls, User Analytics & Financial Monitoring</p>
                    </div>
                </div>
                ...
                <div className="grid md:grid-cols-2 gap-8">
                    ...
                    <div className="bg-card border border-border rounded-2xl p-6 space-y-4">
                        ...
                        <div className="space-y-0 divide-y divide-zinc-800 max-h-[400px] overflow-y-auto">
                                        ...
                                        <div className={`px-2 py-1 rounded text-xs font-bold uppercase
                                            ${log.plan === 'pro' ? 'bg-purple-500/20 text-purple-400' :
                                                log.plan === 'agency' ? 'bg-amber-500/20 text-amber-400' :
                                                    log.plan === 'admin' ? 'bg-red-500/20 text-red-500' : 'bg-muted text-muted-foreground'
                                            }
                                        `}>
                                            {log.plan}
                                        </div>
                            ...
                            {!stats && <div className="text-center text-muted-foreground py-8">Loading or Empty...</div>}
                        </div>
                    </div>
                    ...
                    <div className="bg-card border border-border rounded-2xl p-6 space-y-4">
                        ...
                        <div className="space-y-3">
                            ...
                            {stats?.subscriptions?.map((sub: any) => (
                                <div key={sub.userId} className="flex items-center justify-between p-3 bg-muted/50 rounded-lg border border-zinc-800/50">
                                    <div>
                        ...
                        <div className="mt-8 pt-6 border-t border-border">
                            <h4 className="text-xs font-bold text-muted-foreground uppercase tracking-wider mb-4">System Controls</h4>
                            <div className="grid grid-cols-2 gap-3">
                                <button
                                    onClick={handleFixCors}
                                    className="flex items-center justify-center gap-2 p-3 bg-secondary hover:bg-secondary/80 text-secondary-foreground border border-border text-sm font-medium"
                                >
